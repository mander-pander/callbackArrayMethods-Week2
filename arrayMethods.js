...
/*
  ...
  Instead of just arrays of numbers, we are going to have an array of objects that we want to use
  map, filter, and reduce with.
...
/*
  ...
  (given to you as a tax rate, hint: you'll need to do some multiplication). Your answer
  should be an array of numbers, one total for each order.
*/

const orderTtl = orders.map(order => order.price * (1 + order.tax));
// console.log(orderTtl);
...
const bobsTotal = purchases.filter(purchase => purchase.owner === 'Bob').map(purchase => purchase.price).reduce((acc, curr) => acc +curr);
